...
class Build(object):
    ...
    def sv(self):
        ...
        build_args = [
            "user=sv",
            "BASE_IMAGE=seervision/build:latest",
...
class Start(object):
    ...
    def thib(
        ...
        if mount_nvim_history:
            self._add_mirror_mount(run_args, f"{HOST_HOMEDIR}/.local/share/nvim/lazy")
        if mount_projects_dir:
            self._add_mirror_mount(run_args, f"{HOST_HOMEDIR}/Projects")
        self._add_sv_home_mirror_mount(run_args, ".local/state/nvim/sessions")
        subprocess.run(run_args + [image_tag])
        attach()

    def _add_sv_home_mirror_mount(
        self, run_args: List[str], relative_path: str
    ) -> None:
        self._add_sv_mount(
            run_args, f"{HOST_HOMEDIR}/{relative_path}", f"/home/sv/{relative_path}"
        )
...
def ping(host: str) -> bool:
    ...
    is_reachable = (
        subprocess.call(
            ["fping", "-c", "1", "-t", str(timeout_ms), host],
