...
local set_cwd = function(folder_name, update_vim_cwd)
  ...
  if not update_vim_cwd then vim.cmd("lcd " .. pwd) end
end

local set_cwd_main = function() set_cwd("~/main") end
...
local set_cwd_current_package = function() set_cwd(tros.get_current_package_path()) end
